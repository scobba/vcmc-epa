...
**[my-evaluations/index.html](my-evaluations/index.html) shows one learner their own record**, and is the only page where being signed in does not mean seeing everything. Residents get their EPA averages by rotation, their milestone averages in half-steps, and every evaluation in full — the narrative, and **the attending who wrote it**. Attribution is a program decision taken deliberately on 2026-09-22: a resident should know whose judgement they are reading, and an attending should know their name travels with their feedback. Identity resolves through `resolveFacultyIdentity()` exactly as the dashboard's By Evaluator tab does, so a merged duplicate collapses to one person and an evaluator the roster cannot place is still named from what was typed rather than hidden. Names render through `escFaculty()`, because anonymous self-registration makes `faculty.name` untrusted input.

The isolation is enforced by RLS, not by the page. `epa_submissions` has two SELECT policies — one for program staff, one restricting a resident to rows whose `resident_id` matches their own roster entry — and the page's own `select=*` relies on the database filtering it. See [sql/015_resident_self_service.sql](sql/015_resident_self_service.sql) for the schema (`residents.auth_user_id`, `program_staff`) and, importantly, for the order those policies must be applied in. **Policies are OR'd: adding the resident policy without narrowing the staff one shows every resident the whole program, silently.** The policy bodies are not in this repo.

The page nonetheless checks. If it receives a row whose `resident_id` is not the caller's, it renders nothing and says so, rather than displaying it — cheap insurance against a policy edited wrongly later, and the failure it guards against is not one you can take back. It also refuses to render for an account that matches no roster row, so a staff login lands on an explanation rather than on 108 evaluations belonging to other people.

Verify a policy change the way 015 describes — `set local role authenticated` plus a `request.jwt.claims` sub, inside a transaction that rolls back. That tests isolation without creating an account or exposing anything, and `count(distinct resident_id) = 1` is the assertion that matters, not the row count.

**The date range scopes the whole page, through one array.** `SUBS` holds every evaluation the resident has; `VIEW` holds the ones inside the selected range, and every section — stats, EPA averages, milestones, both charts, the evaluation list — reads `VIEW`. Read `SUBS` in a new section and it silently ignores the filter, so the page shows two different windows of time at once. `renderReport()` rebuilds everything below the range row on each change and leaves the row itself alone, so the date controls are never torn down under the cursor.

The progress charts are hand-drawn SVG, one series each; plotting all nineteen milestones together is unreadable. The dropdowns choose *what* is plotted and do not filter anything — the date range does that. Milestone points are plotted and labelled in half steps, like every other milestone figure. When evaluations share a date, each keeps its own dot, the line runs through their average, and the tooltip lists them all; joining them directly draws a vertical spike that reads as a sudden swing. The marks are `--teal-light` (`#2275AD`), not the page navy: navy fails the palette validator's lightness and chroma checks, so it reads as ink and blends into the axis text. Every chart has a table view, and the tooltip builds attending names with `textContent`.

**Residents sign in with an emailed code, never a link.** Hospital email security opens every link it sees in order to scan it, which spends a single-use Supabase token within seconds of delivery — invitations and reset links both arrived already expired. So the page's one email path is `signInWithOtp` with `shouldCreateUser: false`, and the **Magic Link template in the Supabase dashboard** carries `{{ .Token }}` and deliberately **not** `{{ .ConfirmationURL }}`. The link and the code are the same token, so a link a scanner can open kills the code along with it. That template lives in Supabase, not in this repo: if a sign-in email ever arrives with a button in it, someone has restored the default. "Forgot your password?" goes through the same code screen and then "Choose a password"; nothing sends a reset link. Email goes out through a program Gmail account over custom SMTP, which lifted the built-in service's limit of two emails an hour and is what made the templates editable at all. A rate-limit error is the one send failure the page reports; every other error gets the same neutral answer, so the page cannot be used to test which addresses have accounts. Accounts are created without any email by [tools/provision-resident-accounts.ps1](tools/provision-resident-accounts.ps1), and the site root forwards any auth token that lands on it to `/my-evaluations/`.
...
- The progress charts are duplicated between [my-evaluations/index.html](my-evaluations/index.html) and the By Resident tab of [resident-dashboard/index.html](resident-dashboard/index.html). The dashboard copy is `rdRenderCharts`/`rdDrawChart` and friends, prefixed `rd` and using that page’s `displayEvaluatorName()` and `escFaculty()`. It was copied rather than extracted to `shared/` so as not to refactor the resident page while residents were first using it. Fix a chart bug in one, fix it in the other — or extract both to a shared module.
